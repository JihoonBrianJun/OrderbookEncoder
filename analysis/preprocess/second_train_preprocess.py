...
def main(args):
    ...
    for file in tqdm(files):
        ...
        print(df['mid_price'].diff(periods=args.pred_gap).describe())
        df['gap_mid_price_change'] = df['mid_price'].diff(periods=args.pred_gap) / args.mid_price_change_divisor
        
        src = np.stack([df.iloc[i:i+args.data_len][['qty_ratio', 'maker_ratio']].to_numpy()
                        for i in range(0, df.shape[0]-args.data_len-args.pred_gap, args.data_hop)], axis=0)
        tgt = np.stack([df.iloc[i:i+args.pred_gap]['gap_mid_price_change'].to_numpy()
                        for i in range(args.data_len, df.shape[0]-args.pred_gap, args.data_hop)], axis=0)
